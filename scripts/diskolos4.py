...
class Agent:
    ...
    async def analyze(self, prompt_content: str, max_retries=5, initial_backoff=1) -> Dict[str, Any]:
        retries = 0
        backoff = initial_backoff
        
        while retries <= max_retries:
            try:
                print(f"Starting {self.name} API call (attempt {retries+1}/{max_retries+1})...")
                ...
                if retries >= max_retries:
                    ...
                    if "overloaded" in str(e).lower():
                        return None  # Return None specifically for overload errors
                    return {
                        "agent": self.name,
                        "response": error_msg,
                        "tokens": 0
...
async def write_story():
    ...
    try:
        result = await writer.analyze(prompt, max_retries=5, initial_backoff=1)
        
        if result is None:
            print("Story generation failed due to API overload. Aborting.")
            return None, 0, None
...
Provide corrections if needed and approve for publication if it meets standards of classical Attic Greek."""
    ...
    if result is None:
        print("Story correction failed due to API overload.")
        return None, 0
...
Extract 1-3 challenging words per paragraph with their Latin equivalents."""
    ...
    if result is None:
        print("Vocabulary extraction failed due to API overload.")
        return None, 0
...
async def main():
    ...
    try:
        ...
        if story is None or metadata is None:
            print("Story generation failed. Not creating HTML output.")
            return
        ...
        if corrected_story is None:
            print("Story correction failed. Not creating HTML output.")
            return
        ...
        if vocab_data is None:
            print("Vocabulary extraction failed. Not creating HTML output.")
            return
